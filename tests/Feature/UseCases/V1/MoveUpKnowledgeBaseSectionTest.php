...
final class MoveUpKnowledgeBaseSectionTest extends AbstractTestCase
{
    public static function dataArrayProvider(): iterable
    {
        yield 'move up knowledge base section' => [
            'sectionId' => 10,
            'response' => [
                'kb_section' => [
                    'section_id' => 10,
    ...
    {
        $url = $this->host."/api/kb_section/$sectionId/moveup.json";

        Http::fake([
            $url => Http::response($response),
        ]);

        $payload = new MoveUpKnowledgeBaseSectionPayload($sectionId);
        $responseData = $this->makeHttpClient()->knowledgeBase()->moveUpSection($payload);
        ...
        $this->assertEquals(MoveUpKnowledgeBaseSectionResponse::from($response), $responseData);
    }
}
