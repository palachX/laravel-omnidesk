...
final class EnabledKnowledgeBaseCategoryTest extends AbstractTestCase
{
    public static function dataArrayProvider(): iterable
    {
        yield 'enable knowledge base category' => [
            'categoryId' => 234,
            'response' => [
                'kb_category' => [
                    'category_id' => 234,
    ...
    {
        $url = $this->host."/api/kb_category/$categoryId/enable.json";

        Http::fake([
            $url => Http::response($response),
        ]);

        $responseData = $this->makeHttpClient()->knowledgeBase()->enableCategory($categoryId);
