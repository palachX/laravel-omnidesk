...
final class TrashCaseTest extends AbstractTestCase
{
    ...
    {
        $payload = TrashCasePayload::from($payload);
        $caseId = $payload->caseId;

        $url = $this->host."/api/cases/$caseId/trash.json";

        Http::fake([
            $url => Http::response($response, 200),
        ]);

        $actual = $this->makeHttpClient()->cases()->trashCase($payload);
        ...
        $this->assertEquals(TrashCaseResponse::from($response), $actual);
    }
}
