...
## Disable Idea Category (отключение категории идей)

**Payload:** `Palach\Omnidesk\UseCases\V1\DisableIdeaCategory\Payload`  
**Response:** `Palach\Omnidesk\UseCases\V1\DisableIdeaCategory\Response` (содержит `IdeaCategoryData`).

Отключение категории идей.
...
## Move Up Knowledge Base Article (перемещение статьи базы знаний вверх)

**Response:** `Palach\Omnidesk\UseCases\V1\MoveUpKnowledgeBaseArticle\Response` (содержит `KnowledgeBaseArticleData`).

**HTTP Method:** `PUT /api/kb_article/{id}/moveup.json`

Перемещение статьи базы знаний на шаг вверх.
...
**KnowledgeBaseArticleData** (поле `kb_article` в ответе):
- `article_id` — ID статьи
- `section_id` — ID раздела
...
## Move Down Knowledge Base Article (перемещение статьи базы знаний вниз)

**Response:** `Palach\Omnidesk\UseCases\V1\MoveDownKnowledgeBaseArticle\Response` (содержит `KnowledgeBaseArticleData`).

**HTTP Method:** `PUT /api/kb_article/{id}/movedown.json`

Перемещение статьи базы знаний на шаг вниз.
...
## Delete Knowledge Base Section (удаление раздела)
...
## Delete Knowledge Base Article (удаление статьи)
