...
## Fetch Idea Category List (list idea categories)

**Payload:** `Palach\Omnidesk\UseCases\V1\FetchIdeaCategory\Payload`  
**Response:** `Palach\Omnidesk\UseCases\V1\FetchIdeaCategory\Response` (fields: `ideaCategories` — collection of `IdeaCategoryData`, `total` — total count).

Get a list of idea categories with pagination.
...
| limit | int | 1–100 | Categories per page (default: 100) |
...
## Enable Idea Category (enable idea category)

**Payload:** `Palach\Omnidesk\UseCases\V1\EnableCategory\Payload`  
**Response:** `Palach\Omnidesk\UseCases\V1\EnableIdeaCategory\Response` (contains `IdeaCategoryData`).

Enable an idea category.
...
## Update Knowledge Base Article (update knowledge base article)

**Payload:** `Palach\Omnidesk\UseCases\V1\UpdateKnowledgeBaseArticle\Payload`  
**Response:** `Palach\Omnidesk\UseCases\V1\UpdateKnowledgeBaseArticle\Response` (contains `KnowledgeBaseArticleData`).

**Payload Parameters:**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| article_title | string|array | yes | Article title. If multilingual is enabled, you can pass an array with language IDs as keys and titles as values. |
| article_content | string|array | yes | Article content. If multilingual is enabled, you can pass an array with language IDs as keys and content as values. |
| article_tags | string|array | yes | Keywords for search. If multilingual is enabled, you can pass an array with language IDs. |
| section_id | string|array|int | yes | Section ID(s). You can pass an array to add article to multiple sections. |
| access_type | string | no | Access level (public or staff_only). Default: public. |
...
- `article_tags` — Keywords for search
...
$payload = new UpdateKnowledgeBaseArticlePayload(
    kbArticle: new KnowledgeBaseArticleUpdateData(
        ...
        articleTags: 'updated,tag,article',
        sectionId: 20,
        accessType: 'public'
    )
);
...
$payload = new UpdateKnowledgeBaseArticlePayload(
    kbArticle: new KnowledgeBaseArticleUpdateData(
        ...
        articleTags: 'multi,section,article',
        sectionId: [20, 21, 22],
...
## Fetch Knowledge Base Category List (list knowledge base categories)

**Payload:** `Palach\Omnidesk\UseCases\V1\FetchKnowledgeBaseCategoryList\Payload`  
**Response:** `Palach\Omnidesk\UseCases\V1\FetchKnowledgeBaseCategoryList\Response` (fields: `kbCategories` — collection of `KnowledgeBaseCategoryData`, `total` — total count).

Get a list of knowledge base categories with pagination and language filtering.
...
| page | int | 1–500 | Page number (default: 1) |
| limit | int | 1–100 | Categories per page (default: 100) |
| language_id | string | Optional | Language ID for localized category titles. Use "all" to get all languages. Default: primary language |
...
## Disable Knowledge Base Article (disable knowledge base article)

**Response:** `Palach\Omnidesk\UseCases\V1\DisabledKnowledgeBaseArticle\Response` (contains `KnowledgeBaseArticleData`).
...
$section = $response->kbSection; // KnowledgeBaseSectionData with active = true
```
...
## Move Up Knowledge Base Category (move up knowledge base category)

**Response:** `Palach\Omnidesk\UseCases\V1\MoveUpKnowledgeBaseCategory\Response` (contains `KnowledgeBaseCategoryData`).
...
## Move Up Knowledge Base Section (move up knowledge base section)

**Response:** `Palach\Omnidesk\UseCases\V1\MoveUpKnowledgeBaseSection\Response` (contains `KnowledgeBaseSectionData`).

**HTTP Method:** `PUT /api/kb_section/{id}/moveup.json`

Move up a knowledge base section by one step.
...
## Move Down Knowledge Base Article (move down knowledge base article)

**Response:** `Palach\Omnidesk\UseCases\V1\MoveDownKnowledgeBaseArticle\Response` (contains `KnowledgeBaseArticleData`).

**HTTP Method:** `PUT /api/kb_article/{id}/movedown.json`

Move down a knowledge base article by one step.
...
$response = $knowledgeBase->moveDownArticle(100);
$article = $response->kbArticle; // KnowledgeBaseArticleData
```
...
## Move Down Knowledge Base Category (move down knowledge base category)

**Response:** `Palach\Omnidesk\UseCases\V1\MoveDownKnowledgeBaseCategory\Response` (contains `KnowledgeBaseCategoryData`).
